...
func (tx TxStruct) CreateEmptyTransactionAndHash() (string, string, error) {
	...
	return string(txBytes)+"sequence@"+tx.Sequence, hex.EncodeToString(hash), nil
}
...
func (ts TxStruct) CreateJsonForSend(signature string, pubkey []byte, txType, msgType, pubType string) (*gjson.Result, error) {
	sigBytes, err := hex.DecodeString(signature)
	if err != nil {
		return nil, errors.New("create json fro send failed!")
	}
	gasPrice := strconv.FormatInt(ts.Message[0].GasPrice, 10)
	gasWanted := strconv.FormatInt(ts.Message[0].GasWanted, 10)
	jsonSend := NewJsonSend(txType, msgType, pubType, ts.Memo, ts.Message[0].From, ts.Message[0].To, ts.Message[0].Amount[0].Denom, ts.Message[0].Amount[0].Amount, gasPrice, gasWanted, pubkey, sigBytes)

	json, _ := json.Marshal(jsonSend)
