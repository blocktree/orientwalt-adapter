...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/blocktree/orientwalt-adapter/orientwaltTransaction"
	"math/big"
	"sort"
	"strconv"
	"strings"
...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.CreateHTDFRawTransaction(wrapper, rawTx)
}

//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.SignHTDFRawTransaction(wrapper, rawTx)
}

//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.VerifyHTDFRawTransaction(wrapper, rawTx)
}
...
func (decoder *TransactionDecoder) SubmitRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) (*openwallet.Transaction, error) {
	...
	trxseq := strings.Split(rawTx.RawHex, "sequence@")
	txid, err := decoder.wm.SendRawTransaction(trxseq[0])
	if err != nil {
		fmt.Println("Tx to send: ", rawTx.RawHex)
		return nil, err
	} else {
		sequence,_ := strconv.ParseInt(trxseq[1], 10, 64)
		sequence ++
		//sequence := gjson.Get(rawTx.RawHex, "tx").Get("signatures").Array()[0].Get("sequence").Uint() + 1
		wrapper.SetAddressExtParam(gjson.Get(trxseq[0], "value").Get("msg").Array()[0].Get("value").Get("From").String(), decoder.wm.FullName(), sequence)
	}
...
func (decoder *TransactionDecoder) CreateHTDFRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	for _, a := range addressesBalanceList {
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				log.Error("The HTDF of the account is enough,"+
	...
	txFee := orientwaltTransaction.NewStdFee(int64(decoder.wm.Config.GasPrice), int64(decoder.wm.Config.GasLimit))
	message := []orientwaltTransaction.Message{orientwaltTransaction.NewMessage(from, to, denom, int64(convertFromAmount(amountStr)), int64(decoder.wm.Config.GasPrice), int64(decoder.wm.Config.GasLimit))}
	txStruct := orientwaltTransaction.NewTxStruct(chainID, memo, accountNumber, int(sequence), &txFee, message)
...
func (decoder *TransactionDecoder) SignHTDFRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			/////////交易单哈希签名
			sig, err := orientwaltTransaction.SignTransactionHash(keySignature.Message, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyHTDFRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	pass := orientwaltTransaction.VerifyTransactionSig(emptyTrans, signature, point)
	trxseq := strings.Split(emptyTrans, "sequence@")

	var txStruct orientwaltTransaction.TxStruct
	json.Unmarshal([]byte(trxseq[0]), &txStruct)
	keyType := "tendermint/PubKeySecp256k1"
	signedTrans, err := txStruct.CreateJsonForSend(signature, pubkey, decoder.wm.Config.TxType, decoder.wm.Config.MsgType, keyType)
	...
	if pass {
		...
		rawTx.RawHex = signedTrans.Raw + "sequence@" + trxseq[1]
	} else {
...
func (decoder *TransactionDecoder) createRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction, addrBalance *openwallet.Balance) error {
	...
	fee := decoder.wm.Config.FixedFee //decoder.wm.Config.FeeCharge
