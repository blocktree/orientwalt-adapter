...
func (c *Client) getBlockHash(height uint64) (string, error) {
	...
	path := fmt.Sprintf("/block_detail/%d", height)
...
func (c *Client) getBalance(address string, denom string) (*AddrBalance, error) {
	...
	for _, coin := range coins {
		...
		if coin.Get("denom").String() == "htdf" {
			return &AddrBalance{Address:address, Balance:big.NewInt(int64(convertFromAmount(coin.Get("amount").String())))}, nil
		}
	}
...
func (c *Client) sendTransaction(jsonStr string) (string, error) {
	...
	dat["tx"] = hex.EncodeToString([]byte(jsonStr))
