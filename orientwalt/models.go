...
func NewTransaction(json *gjson.Result, txType, msgType, denom string) *Transaction {
			...
			for _, coin := range msg.Get("value").Get("Amount").Array() {
				if coin.Get("denom").String() == denom {
					obj.TxValue = append(obj.TxValue, TxValue{
						From:   msg.Get("value").Get("From").String(),
						To:     msg.Get("value").Get("To").String(),
	...
	feeJson := json.Get("tx").Get("value").Get("fee")
	obj.Fee = append(obj.Fee, FeeValue{Amount: feeJson.Get("gas_wanted").Uint() * feeJson.Get("gas_price").Uint()})
...
func NewBlock(json *gjson.Result) *Block {
	...
	if gjson.Get(json.Raw, "block_meta").Get("header").Get("num_txs").Uint() != 0 {
		txs := gjson.Get(json.Raw, "block").Get("txs").Array()
		...
		for _, tx := range txs {
			...
			obj.Transactions = append(obj.Transactions, tx.Get("Hash").String())
		}
	}
